...
def run_fit(xData, yData):
    # Single Lorentzian peak function
    # bounds on parameters are set in generate_Initial_Parameters() below

    def fit_func(x, a, b, A0, w0, x0):
        return a + x * b + (2 * A0 / np.pi) * (w0 / (4 * (x - x0) ** 2 + w0 ** 2))
    ...
    a, b, A0, w0, x0 = fittedParameters
    y_fit = fit_func(xData, a, b, A0, w0, x0)
