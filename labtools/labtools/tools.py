...
def correlation(data, corr_window=300_000, start_chs=[1], stop_chs=[4], trig_ch=5):
    ...
    while True:#shift != 20:
        shift += 1
        roll_1 = np.roll(data, shift)
        roll_1['time'] = data['time'] - roll_1['time']
        diffs_1 = roll_1['time'][(roll_1['time'] < corr_window) & np.isin(data['channel'], start_chs) & np.isin(roll_1['channel'], stop_chs)]
        autoc_diffs.append(diffs_1.astype('int32'))
        diffs_2 = roll_1['time'][(roll_1['time'] < corr_window) & np.isin(data['channel'], stop_chs) & np.isin(roll_1['channel'], start_chs)]
