...
class Window(QMainWindow):
    ...
    def newMeasurement(self):
        ...
        print(self.tag.currentChan)
        if len(self.tag.currentChan)>1:
            self.tag.hist_1 = Histogram(self.tag.tagger, self.tag.comb_ch, self.tag.trigChans[0], self.tag.binWidth, self.tag.numBins)
        else:
            self.tag.hist_1 = Histogram(self.tag.tagger, self.tag.currentChan[0], self.tag.trigChans[0], self.tag.binWidth, self.tag.numBins)

        if len(self.tag.currentChan2)>1:
            self.tag.hist_2 = Histogram(self.tag.tagger, self.tag.comb_ch, self.tag.trigChans[0], self.tag.binWidth2, self.tag.numBins2)
        else:
            self.tag.hist_2 = Histogram(self.tag.tagger, self.tag.currentChan2[0], self.tag.trigChans[0], self.tag.binWidth2, self.tag.numBins2)
        ...
        self.tag.counter = Counter(self.tag.tagger, self.tag.apdChans, self.tag.binWidthCounter, self.tag.numBinsCounter)
        region = self.m_ui.lr41.getRegion()

        self.init_params_ui()

        #self.tag.stream.getData(self.tag.buf)
        #current_data_times = self.tag.buf.getTimestamps()
        #current_data_channels = self.tag.buf.getChannels()
        #self.data_times = np.append(self.data_times, current_data_times)
        #self.data_channels = np.append(self.data_channels, current_data_channels)

        #self.trig_len = self.get_trig_len(self.data_times, self.data_channels)

        #ax = self.p1.getAxis('bottom')
        #ax2 = self.p1_2.getAxis('bottom')
        #ticks = np.linspace(0, self.trig_len, 10)
        #ax.setTicks([[(v, str(v)) for v in ticks ]])
        #ax2.setTicks([[(v, str(v)) for v in ticks ]])
        #self.curve_hist_1 = self.p1.plot()
        #self.curve_hist_2 = self.p1_2.plot()
        #print("Trig_len: ", self.trig_len)
...
class TT:
    def __init__(self, parameters):
        ...
        self.virtChannel = DelayedChannel(self.tagger, input_channel=self.trigChans[0], delay=self.delayTimes[8]).getChannel()
        self.gated_channel_1 = GatedChannel(self.tagger, input_channel=1, gate_start_channel = self.virtChannel, gate_stop_channel = self.trigChans[0])
        ...
        self.hist_2 = Histogram(self.tagger, self.currentChan2[0], self.trigChans[0], self.binWidth2, self.numBins2)
        ...
        self.counter = Counter(self.tagger,self.apdChans, self.binWidthCounter, self.numBinsCounter)
